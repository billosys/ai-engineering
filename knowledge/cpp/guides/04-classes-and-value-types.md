...
- `s-class` — C: Classes and class hierarchies (source lines 4296-9143)
- `s-enum` — Enum: Enumerations (source lines 9144-9413)

## Rule Index

- `C.1` — Organize related data into structures (`struct`s or `class`es) (`rc-org`, source line 4322)
- `C.2` — Use `class` if the class has an invariant; use `struct` if the data members can vary independently (`rc-struct`, source line 4346)
- `C.3` — Represent the distinction between an interface and an implementation using a class (`rc-interface`, source line 4400)
- `C.4` — Make a function a member only if it needs direct access to the representation of a class (`rc-member`, source line 4434)
- `C.5` — Place helper functions in the same namespace as the class they support (`rc-helper`, source line 4496)
- `C.7` — Don't define a class or enum and declare a variable of its type in the same statement (`rc-standalone`, source line 4524)
- `C.8` — Use `class` rather than `struct` if any member is non-public (`rc-class`, source line 4543)
- `C.9` — Minimize exposure of members (`rc-private`, source line 4577)
- `C.10` — Prefer concrete types over class hierarchies (`rc-concrete`, source line 4678)
- `C.11` — Make concrete types regular (`rc-regular`, source line 4730)
- `C.12` — Don't make data members `const` or references in a copyable or movable type (`rc-constref`, source line 4772)
- `C.13` — If data member `B` uses another data member `A`, declare `A` before `B` (`rc-lifetime`, source line 4797)
- `C.20` — If you can avoid defining default operations, do (`rc-zero`, source line 4974)
- `C.21` — If you define or `=delete` any copy, move, or destructor function, define or `=delete` them all (`rc-five`, source line 5007)
- `C.22` — Make default operations consistent (`rc-matched`, source line 5115)
- `C.30` — Define a destructor if a class needs an explicit action at object destruction (`rc-dtor`, source line 5151)
- `C.31` — All resources acquired by a class must be released by the class's destructor (`rc-dtor-release`, source line 5209)
- `C.32` — If a class has a raw pointer (`T*`) or reference (`T&`), consider whether it might be owning (`rc-dtor-ptr`, source line 5266)
- `C.33` — If a class has an owning pointer member, define a destructor (`rc-dtor-ptr2`, source line 5291)
- `C.35` — A base class destructor should be either public and virtual, or protected and non-virtual (`rc-dtor-virtual`, source line 5365)
- `C.36` — A destructor must not fail (`rc-dtor-fail`, source line 5426)
- `C.37` — Make destructors `noexcept` (`rc-dtor-noexcept`, source line 5480)
- `C.40` — Define a constructor if a class has an invariant (`rc-ctor`, source line 5515)
- `C.41` — A constructor should create a fully initialized object (`rc-complete`, source line 5573)
- `C.42` — If a constructor cannot construct a valid object, throw an exception (`rc-throw`, source line 5616)
- `C.43` — Ensure that a copyable class has a default constructor (`rc-default0`, source line 5705)
- `C.44` — Prefer default constructors to be simple and non-throwing (`rc-default00`, source line 5825)
- `C.45` — Don't define a default constructor that only initializes data members; use default member initializers instead (`rc-default`, source line 5873)
- `C.46` — By default, declare single-argument constructors explicit (`rc-explicit`, source line 5903)
- `C.47` — Define and initialize data members in the order of member declaration (`rc-order`, source line 5941)
- `C.48` — Prefer default member initializers to member initializers in constructors for constant initializers (`rc-in-class-initializer`, source line 5965)
- `C.49` — Prefer initialization to assignment in constructors (`rc-initialize`, source line 6014)
- `C.50` — Use a factory function if you need "virtual behavior" during initialization (`rc-factory`, source line 6057)
- `C.51` — Use delegating constructors to represent common actions for all constructors of a class (`rc-delegating`, source line 6128)
- `C.52` — Use inheriting constructors to import constructors into a derived class that does not need further explicit initialization (`rc-inheriting`, source line 6175)
- `C.60` — Make copy assignment non-`virtual`, take the parameter by `const&`, and return by non-`const&` (`rc-copy-assignment`, source line 6215)
- `C.61` — A copy operation should copy (`rc-copy-semantic`, source line 6285)
- `C.62` — Make copy assignment safe for self-assignment (`rc-copy-self`, source line 6356)
- `C.63` — Make move assignment non-`virtual`, take the parameter by `&&`, and return by non-`const&` (`rc-move-assignment`, source line 6422)
- `C.64` — A move operation should move and leave its source in a valid state (`rc-move-semantic`, source line 6438)
- `C.65` — Make move assignment safe for self-assignment (`rc-move-self`, source line 6492)
- `C.66` — Make move operations `noexcept` (`rc-move-noexcept`, source line 6541)
- `C.67` — A polymorphic class should suppress public copy/move (`rc-copy-virtual`, source line 6589)
- `C.80` — Use `=default` if you have to be explicit about using the default semantics (`rc-eqdefault`, source line 6663)
- `C.81` — Use `=delete` when you want to disable default behavior (without wanting an alternative) (`rc-delete`, source line 6705)
- `C.82` — Don't call virtual functions in constructors and destructors (`rc-ctor-virtual`, source line 6757)
- `C.83` — For value-like types, consider providing a `noexcept` swap function (`rc-swap`, source line 6810)
- `C.84` — A `swap` function must not fail (`rc-swap-fail`, source line 6842)
- `C.85` — Make `swap` `noexcept` (`rc-swap-noexcept`, source line 6863)
- `C.86` — Make `==` symmetric with respect to operand types and `noexcept` (`rc-eq`, source line 6874)
- `C.87` — Beware of `==` on base classes (`rc-eq-base`, source line 6919)
- `C.89` — Make a `hash` `noexcept` (`rc-hash`, source line 6969)
- `C.90` — Rely on constructors and assignment operators, not `memset` and `memcpy` (`rc-memset`, source line 7006)
- `C.100` — Follow the STL when defining a container (`rcon-stl`, source line 7064)
- `C.101` — Give a container value semantics (`rcon-val`, source line 7125)
- `C.102` — Give a container move operations (`rcon-move`, source line 7151)
- `C.103` — Give a container an initializer list constructor (`rcon-init`, source line 7175)
- `C.104` — Give a container a default constructor that sets it to empty (`rcon-empty`, source line 7190)
- `C.109` — If a resource handle has pointer semantics, provide `*` and `->` (`rcon-ptr`, source line 7204)
- `C.120` — Use class hierarchies to represent concepts with inherent hierarchical structure (only) (`rh-domain`, source line 7276)
- `C.121` — If a base class is used as an interface, make it a pure abstract class (`rh-abstract`, source line 7341)
- `C.122` — Use abstract classes as interfaces when complete separation of interface and implementation is needed (`rh-separation`, source line 7384)
- `C.126` — An abstract class typically doesn't need a user-written constructor (`rh-abstract-ctor`, source line 7421)
- `C.127` — A class with a virtual function should have a virtual or protected destructor (`rh-dtor`, source line 7454)
- `C.128` — Virtual functions should specify exactly one of `virtual`, `override`, or `final` (`rh-override`, source line 7488)
- `C.129` — When designing a class hierarchy, distinguish between implementation inheritance and interface inheritance (`rh-kind`, source line 7544)
- `C.130` — For making deep copies of polymorphic classes prefer a virtual `clone` function instead of public copy construction/assignment (`rh-copy`, source line 7776)
- `C.131` — Avoid trivial getters and setters (`rh-get`, source line 7809)
- `C.132` — Don't make a function `virtual` without reason (`rh-virtual`, source line 7846)
- `C.133` — Avoid `protected` data (`rh-protected`, source line 7873)
- `C.134` — Ensure all non-`const` data members have the same access level (`rh-public`, source line 7919)
- `C.135` — Use multiple inheritance to represent multiple distinct interfaces (`rh-mi-interface`, source line 7954)
- `C.136` — Use multiple inheritance to represent the union of implementation attributes (`rh-mi-implementation`, source line 7983)
- `C.137` — Use `virtual` bases to avoid overly general base classes (`rh-vbase`, source line 8014)
- `C.138` — Create an overload set for a derived class and its bases with `using` (`rh-using`, source line 8067)
- `C.139` — Use `final` on classes sparingly (`rh-final`, source line 8116)
- `C.140` — Do not provide different default arguments for a virtual function and an overrider (`rh-virtual-default-arg`, source line 8152)
- `C.145` — Access polymorphic objects through pointers and references (`rh-poly`, source line 8183)
- `C.146` — Use `dynamic_cast` where class hierarchy navigation is unavoidable (`rh-dynamic_cast`, source line 8227)
- `C.147` — Use `dynamic_cast` to a reference type when failure to find the required class is considered an error (`rh-ref-cast`, source line 8364)
- `C.148` — Use `dynamic_cast` to a pointer type when failure to find the required class is considered a valid alternative (`rh-ptr-cast`, source line 8381)
- `C.149` — Use `unique_ptr` or `shared_ptr` to avoid forgetting to `delete` objects created using `new` (`rh-smart`, source line 8421)
- `C.150` — Use `make_unique()` to construct objects owned by `unique_ptr`s (`rh-make_unique`, source line 8442)
- `C.151` — Use `make_shared()` to construct objects owned by `shared_ptr`s (`rh-make_shared`, source line 8446)
- `C.152` — Never assign a pointer to an array of derived class objects to a pointer to its base (`rh-array`, source line 8450)
- `C.153` — Prefer virtual function to casting (`rh-use-virtual`, source line 8475)
- `C.160` — Define operators primarily to mimic conventional usage (`ro-conventional`, source line 8509)
- `C.161` — Use non-member functions for symmetric operators (`ro-symmetric`, source line 8541)
- `C.162` — Overload operations that are roughly equivalent (`ro-equivalent`, source line 8556)
- `C.163` — Overload only for operations that are roughly equivalent (`ro-equivalent-2`, source line 8582)
- `C.164` — Avoid implicit conversion operators (`ro-conversion`, source line 8611)
- `C.165` — Use `using` for customization points (`ro-custom`, source line 8660)
- `C.166` — Overload unary `&` only as part of a system of smart pointers and references (`ro-address-of`, source line 8707)
- `C.167` — Use an operator for an operation with its conventional meaning (`ro-overload`, source line 8741)
- `C.168` — Define overloaded operators in the namespace of their operands (`ro-namespace`, source line 8780)
- `C.170` — If you feel like overloading a lambda, use a generic lambda (`ro-lambda`, source line 8843)
- `C.180` — Use `union`s to save memory (`ru-union`, source line 8880)
- `C.181` — Avoid "naked" `union`s (`ru-naked`, source line 8946)
- `C.182` — Use anonymous `union`s to implement tagged unions (`ru-anonymous`, source line 8993)
- `C.183` — Don't use a `union` for type punning (`ru-pun`, source line 9094)
- `Enum.1` — Prefer enumerations over macros (`renum-macro`, source line 9160)
- `Enum.2` — Use enumerations to represent sets of related named constants (`renum-set`, source line 9202)
- `Enum.3` — Prefer class enums over "plain" enums (`renum-class`, source line 9237)
- `Enum.4` — Define operations on enumerations for safe and simple use (`renum-oper`, source line 9271)
- `Enum.5` — Don't use `ALL_CAPS` for enumerators (`renum-caps`, source line 9304)
- `Enum.6` — Avoid unnamed enumerations (`renum-unnamed`, source line 9326)
- `Enum.7` — Specify the underlying type of an enumeration only when necessary (`renum-underlying`, source line 9351)
- `Enum.8` — Specify enumerator values only when necessary (`renum-value`, source line 9389)
