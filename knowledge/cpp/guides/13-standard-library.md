...
- `s-stdlib` — SL: The Standard Library (source lines 19821-20570)
- `s-libraries` — Appendix A: Libraries (source lines 22262-22267)

## Rule Index

- `SL.1` — Use libraries wherever possible (`rsl-lib`, source line 19849)
- `SL.2` — Prefer the standard library to other libraries (`rsl-sl`, source line 19858)
- `SL.3` — Do not add non-standard entities to namespace `std` (`sl-std`, source line 19866)
- `SL.4` — Use the standard library in a type-safe manner (`sl-safe`, source line 19895)
- `SL.con.1` — Prefer using STL `array` or `vector` instead of a C array (`rsl-arrays`, source line 19922)
- `SL.con.2` — Prefer using STL `vector` by default unless you have a reason to use a different container (`rsl-vector`, source line 19959)
- `SL.con.3` — Avoid bounds errors (`rsl-bounds`, source line 19999)
- `SL.con.4` — don't use `memset` or `memcpy` for arguments that are not trivially-copyable (`rsl-copy`, source line 20073)
- `SL.str.1` — Use `std::string` to own character sequences (`rstr-string`, source line 20152)
- `SL.str.2` — Use `std::string_view` or `gsl::span<char>` to refer to character sequences (`rstr-view`, source line 20211)
- `SL.str.3` — Use `zstring` or `czstring` to refer to a C-style, zero-terminated, sequence of characters (`rstr-zstring`, source line 20238)
- `SL.str.4` — Use `char*` to refer to a single character (`rstr-charp`, source line 20278)
- `SL.str.5` — Use `std::byte` to refer to byte values that do not necessarily represent characters (`rstr-byte`, source line 20308)
- `SL.str.10` — Use `std::string` when you need to perform locale-sensitive string operations (`rstr-locale`, source line 20328)
- `SL.str.11` — Use `gsl::span<char>` rather than `std::string_view` when you need to mutate a string (`rstr-span`, source line 20346)
- `SL.str.12` — Use the `s` suffix for string literals meant to be standard-library `string`s (`rstr-s`, source line 20364)
- `SL.io.1` — Use character-level input only when you have to (`rio-low`, source line 20400)
- `SL.io.2` — When reading, always consider ill-formed input (`rio-validate`, source line 20431)
- `SL.io.3` — Prefer `iostream`s for I/O (`rio-streams`, source line 20446)
- `SL.io.10` — Unless you use `printf`-family functions call `ios_base::sync_with_stdio(false)` (`rio-sync`, source line 20487)
- `SL.io.50` — Avoid `endl` (`rio-endl`, source line 20506)
- `SL.C.1` — Don't use setjmp/longjmp (`rclib-jmp`, source line 20559)
