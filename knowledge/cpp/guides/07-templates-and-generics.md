...
- `s-templates` — T: Templates and generic programming (source lines 17130-19177)

## Rule Index

- `T.1` — Use templates to raise the level of abstraction of code (`rt-raise`, source line 17236)
- `T.2` — Use templates to express algorithms that apply to many argument types (`rt-algo`, source line 17297)
- `T.3` — Use templates to express containers and ranges (`rt-cont`, source line 17324)
- `T.4` — Use templates to express syntax tree manipulation (`rt-expr`, source line 17366)
- `T.5` — Combine generic and OO techniques to amplify their strengths, not their costs (`rt-generic-oo`, source line 17378)
- `T.10` — Specify concepts for all template arguments (`rt-concepts`, source line 17489)
- `T.11` — Whenever possible use standard concepts (`rt-std-concepts`, source line 17529)
- `T.12` — Prefer concept names over `auto` for local variables (`rt-auto`, source line 17569)
- `T.13` — Prefer the shorthand notation for simple, single-type argument concepts (`rt-shorthand`, source line 17585)
- `T.20` — Avoid "concepts" without meaningful semantics (`rt-low`, source line 17621)
- `T.21` — Require a complete set of operations for a concept (`rt-complete`, source line 17688)
- `T.22` — Specify axioms for concepts (`rt-axiom`, source line 17778)
- `T.23` — Differentiate a refined concept from its more general case by adding new use patterns. (`rt-refine`, source line 17841)
- `T.24` — Use tag classes or traits to differentiate concepts that differ only in semantics. (`rt-tag`, source line 17867)
- `T.25` — Avoid complementary constraints (`rt-not`, source line 17906)
- `T.26` — Prefer to define concepts in terms of use-patterns rather than simple syntax (`rt-use`, source line 17972)
- `T.40` — Use function objects to pass operations to algorithms (`rt-fo`, source line 18009)
- `T.41` — Require only essential properties in a template's concepts (`rt-essential`, source line 18047)
- `T.42` — Use template aliases to simplify notation and hide implementation details (`rt-alias`, source line 18102)
- `T.43` — Prefer `using` over `typedef` for defining aliases (`rt-using`, source line 18155)
- `T.44` — Use function templates to deduce class template argument types (where feasible) (`rt-deduce`, source line 18179)
- `T.46` — (removed) (`rt-regular`, source line 18215)
- `T.47` — Avoid highly visible unconstrained templates with common names (`rt-visible`, source line 18218)
- `T.48` — If your compiler does not support concepts, fake them with `enable_if` (`rt-concept-def`, source line 18271)
- `T.49` — Where possible, avoid type-erasure (`rt-erasure`, source line 18303)
- `T.60` — Minimize a template's context dependencies (`rt-depend`, source line 18329)
- `T.61` — Do not over-parameterize members (SCARY) (`rt-scary`, source line 18372)
- `T.62` — Place non-dependent class template members in a non-templated base class (`rt-nondependent`, source line 18443)
- `T.64` — Use specialization to provide alternative implementations of class templates (`rt-specialization`, source line 18483)
- `T.65` — Use tag dispatch to provide alternative implementations of a function (`rt-tag-dispatch`, source line 18504)
- `T.67` — Use specialization to provide alternative implementations for irregular types (`rt-specialization2`, source line 18573)
- `T.68` — Use `{}` rather than `()` within templates to avoid ambiguities (`rt-cast`, source line 18587)
- `T.69` — Inside a template, don't make an unqualified non-member function call unless you intend it to be a customization point (`rt-customization`, source line 18611)
- `T.80` — Do not naively templatize a class hierarchy (`rt-hier`, source line 18667)
- `T.81` — Do not mix hierarchies and arrays (`rt-array`, source line 18708)
- `T.82` — Linearize a hierarchy when virtual functions are undesirable (`rt-linear`, source line 18757)
- `T.83` — Do not declare a member function template virtual (`rt-virtual`, source line 18771)
- `T.84` — Use a non-template core implementation to provide an ABI-stable interface (`rt-abi`, source line 18799)
- `T.100` — Use variadic templates when you need a function that takes a variable number of arguments of a variety of types (`rt-variadic`, source line 18853)
- `T.101` — ??? How to pass arguments to a variadic template ??? (`rt-variadic-pass`, source line 18867)
- `T.102` — How to process arguments to a variadic template (`rt-variadic-process`, source line 18881)
- `T.103` — Don't use variadic templates for homogeneous argument lists (`rt-variadic-not`, source line 18895)
- `T.120` — Use template metaprogramming only when you really need to (`rt-metameta`, source line 18917)
- `T.121` — Use template metaprogramming primarily to emulate concepts (`rt-emulate`, source line 18946)
- `T.122` — Use templates (usually template aliases) to compute types at compile time (`rt-tmp`, source line 18975)
- `T.123` — Use `constexpr` functions to compute values at compile time (`rt-fct`, source line 18993)
- `T.124` — Prefer to use standard-library TMP facilities (`rt-std-tmp`, source line 19021)
- `T.125` — If you need to go beyond the standard-library TMP facilities, use an existing library (`rt-lib`, source line 19035)
- `T.140` — If an operation can be reused, give it a name (`rt-name`, source line 19052)
- `T.141` — Use an unnamed lambda if you need a simple function object in one place only (`rt-lambda`, source line 19056)
- `T.143` — Don't write unintentionally non-generic code (`rt-non-generic`, source line 19074)
- `T.144` — Don't specialize function templates (`rt-specialize-function`, source line 19134)
- `T.150` — Check that a class matches a concept using `static_assert` (`rt-check-class`, source line 19151)
