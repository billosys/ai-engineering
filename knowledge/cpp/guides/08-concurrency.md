...
- `s-concurrency` — CP: Concurrency and parallelism (source lines 14289-15908)

## Rule Index

- `CP.1` — Assume that your code will run as part of a multi-threaded program (`rconc-multi`, source line 14346)
- `CP.2` — Avoid data races (`rconc-races`, source line 14416)
- `CP.3` — Minimize explicit sharing of writable data (`rconc-data`, source line 14508)
- `CP.4` — Think in terms of tasks, rather than threads (`rconc-task`, source line 14549)
- `CP.8` — Don't try to use `volatile` for synchronization (`rconc-volatile`, source line 14578)
- `CP.9` — Whenever feasible use tools to validate your concurrent code (`rconc-tools`, source line 14630)
- `CP.20` — Use RAII, never plain `lock()`/`unlock()` (`rconc-raii`, source line 14700)
- `CP.21` — Use `std::lock()` or `std::scoped_lock` to acquire multiple `mutex`es (`rconc-lock`, source line 14732)
- `CP.22` — Never call unknown code while holding a lock (e.g., a callback) (`rconc-unknown`, source line 14791)
- `CP.23` — Think of a joining `thread` as a scoped container (`rconc-join`, source line 14835)
- `CP.24` — Think of a `thread` as a global container (`rconc-detach`, source line 14873)
- `CP.25` — Prefer `gsl::joining_thread` over `std::thread` (`rconc-joining_thread`, source line 14933)
- `CP.26` — Don't `detach()` a thread (`rconc-detached_thread`, source line 14989)
- `CP.31` — Pass small amounts of data between threads by value, rather than by reference or pointer (`rconc-data-by-value`, source line 15042)
- `CP.32` — To share ownership between unrelated `thread`s use `shared_ptr` (`rconc-shared`, source line 15079)
- `CP.40` — Minimize context switching (`rconc-switch`, source line 15102)
- `CP.41` — Minimize thread creation and destruction (`rconc-create`, source line 15117)
- `CP.42` — Don't `wait` without a condition (`rconc-wait`, source line 15173)
- `CP.43` — Minimize time spent in a critical section (`rconc-time`, source line 15243)
- `CP.44` — Remember to name your `lock_guard`s and `unique_lock`s (`rconc-name`, source line 15292)
- `CP.50` — Define a `mutex` together with the data it guards. Use `synchronized_value<T>` where possible (`rconc-mutex`, source line 15323)
- `CP.51` — Do not use capturing lambdas that are coroutines (`rcoro-capture`, source line 15361)
- `CP.52` — Do not hold locks or other synchronization primitives across suspension points (`rcoro-locks`, source line 15419)
- `CP.53` — Parameters to coroutines should not be passed by reference (`rcoro-reference-parameters`, source line 15461)
- `CP.60` — Use a `future` to return a value from a concurrent task (`rconc-future`, source line 15529)
- `CP.61` — Use `async()` to spawn concurrent tasks (`rconc-async`, source line 15548)
- `CP.100` — Don't use lock-free programming unless you absolutely have to (`rconc-lockfree`, source line 15665)
- `CP.101` — Distrust your hardware/compiler combination (`rconc-distrust`, source line 15698)
- `CP.102` — Carefully study the literature (`rconc-literature`, source line 15718)
- `CP.110` — Do not write your own double-checked locking for initialization (`rconc-double`, source line 15738)
- `CP.111` — Use a conventional pattern if you really need double-checked locking (`rconc-double-pattern`, source line 15781)
- `CP.200` — Use `volatile` only to talk to non-C++ memory (`rconc-volatile2`, source line 15842)
- `CP.201` — ??? Signals (`rconc-signal`, source line 15904)
