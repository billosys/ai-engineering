...
- `s-resource` — R: Resource management (source lines 9414-10254)
- `s-gsl` — GSL: Guidelines support library (source lines 21325-21472)

## Rule Index

- `R.1` — Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization) (`rr-raii`, source line 9461)
- `R.2` — In interfaces, use raw pointers to denote individual objects (only) (`rr-use-ptr`, source line 9523)
- `R.3` — A raw pointer (a `T*`) is non-owning (`rr-ptr`, source line 9565)
- `R.4` — A raw reference (a `T&`) is non-owning (`rr-ref`, source line 9670)
- `R.5` — Prefer scoped objects, don't heap-allocate unnecessarily (`rr-scoped`, source line 9692)
- `R.6` — Avoid non-`const` global variables (`rr-global`, source line 9729)
- `R.10` — Avoid `malloc()` and `free()` (`rr-mallocfree`, source line 9735)
- `R.11` — Avoid calling `new` and `delete` explicitly (`rr-newdelete`, source line 9782)
- `R.12` — Immediately give the result of an explicit resource allocation to a manager object (`rr-immediate-alloc`, source line 9800)
- `R.13` — Perform at most one explicit resource allocation in a single expression statement (`rr-single-alloc`, source line 9833)
- `R.14` — Avoid `[]` parameters, prefer `span` (`rr-ap`, source line 9869)
- `R.15` — Always overload matched allocation/deallocation pairs (`rr-pair`, source line 9889)
- `R.20` — Use `unique_ptr` or `shared_ptr` to represent ownership (`rr-owner`, source line 9915)
- `R.21` — Prefer `unique_ptr` over `shared_ptr` unless you need to share ownership (`rr-unique`, source line 9939)
- `R.22` — Use `make_shared()` to make `shared_ptr`s (`rr-make_shared`, source line 9969)
- `R.23` — Use `make_unique()` to make `unique_ptr`s (`rr-make_unique`, source line 9990)
- `R.24` — Use `std::weak_ptr` to break cycles of `shared_ptr`s (`rr-weak_ptr`, source line 10007)
- `R.30` — Take smart pointers as parameters only to explicitly express lifetime semantics (`rr-smartptrparam`, source line 10054)
- `R.31` — If you have non-`std` smart pointers, follow the basic pattern from `std` (`rr-smart`, source line 10058)
- `R.32` — Take a `unique_ptr<widget>` parameter to express that a function assumes ownership of a `widget` (`rr-uniqueptrparam`, source line 10091)
- `R.33` — Take a `unique_ptr<widget>&` parameter to express that a function reseats the `widget` (`rr-reseat`, source line 10107)
- `R.34` — Take a `shared_ptr<widget>` parameter to express shared ownership (`rr-sharedptrparam-owner`, source line 10125)
- `R.35` — Take a `shared_ptr<widget>&` parameter to express that a function might reseat the shared pointer (`rr-sharedptrparam`, source line 10150)
- `R.36` — Take a `const shared_ptr<widget>&` parameter to express that it might retain a reference count to the object ??? (`rr-sharedptrparam-const`, source line 10174)
- `R.37` — Do not pass a pointer or reference obtained from an aliased smart pointer (`rr-smartptrget`, source line 10194)
