...
- `s-errors` — E: Error handling (source lines 15909-16928)

## Rule Index

- `E.1` — Develop an error-handling strategy early in a design (`re-design`, source line 15957)
- `E.2` — Throw an exception to signal that a function can't perform its assigned task (`re-throw`, source line 15963)
- `E.3` — Use exceptions for error handling only (`re-errors`, source line 16030)
- `E.4` — Design your error-handling strategy around invariants (`re-design-invariants`, source line 16060)
- `E.5` — Let a constructor establish an invariant, and throw if it cannot (`re-invariant`, source line 16074)
- `E.6` — Use RAII to prevent leaks (`re-raii`, source line 16106)
- `E.7` — State your preconditions (`re-precondition`, source line 16218)
- `E.8` — State your postconditions (`re-postcondition`, source line 16226)
- `E.12` — Use `noexcept` when exiting a function because of a `throw` is impossible or unacceptable (`re-noexcept`, source line 16234)
- `E.13` — Never throw while being the direct owner of an object (`re-never-throw`, source line 16273)
- `E.14` — Use purpose-designed user-defined types as exceptions (not built-in types) (`re-exception-types`, source line 16313)
- `E.15` — Throw by value, catch exceptions from a hierarchy by reference (`re-exception-ref`, source line 16368)
- `E.16` — Destructors, deallocation, `swap`, and exception type copy/move construction must never fail (`re-never-fail`, source line 16411)
- `E.17` — Don't try to catch every exception in every function (`re-not-always`, source line 16458)
- `E.18` — Minimize the use of explicit `try`/`catch` (`re-catch`, source line 16484)
- `E.19` — Use a `final_action` object to express cleanup if no suitable resource handle is available (`re-finally`, source line 16527)
- `E.25` — If you can't throw exceptions, simulate RAII for resource management (`re-no-throw-raii`, source line 16557)
- `E.26` — If you can't throw exceptions, consider failing fast (`re-no-throw-crash`, source line 16618)
- `E.27` — If you can't throw exceptions, use error codes systematically (`re-no-throw-codes`, source line 16661)
- `E.28` — Avoid error handling based on global state (e.g. `errno`) (`re-no-throw`, source line 16827)
- `E.30` — Don't use exception specifications (`re-specifications`, source line 16857)
- `E.31` — Properly order your `catch`-clauses (`re_catch`, source line 16902)
