...
- `s-interfaces` — I: Interfaces (source lines 1215-2330)

## Rule Index

- `I.1` — Make interfaces explicit (`ri-explicit`, source line 1253)
- `I.2` — Avoid non-`const` global variables (`ri-global`, source line 1297)
- `I.3` — Avoid singletons (`ri-singleton`, source line 1361)
- `I.4` — Make interfaces precisely and strongly typed (`ri-typed`, source line 1422)
- `I.5` — State preconditions (if any) (`ri-pre`, source line 1544)
- `I.6` — Prefer `Expects()` for expressing preconditions (`ri-expects`, source line 1585)
- `I.7` — State postconditions (`ri-post`, source line 1623)
- `I.8` — Prefer `Ensures()` for expressing postconditions (`ri-ensures`, source line 1719)
- `I.9` — If an interface is a template, document its parameters using concepts (`ri-concepts`, source line 1752)
- `I.10` — Use exceptions to signal a failure to perform a required task (`ri-except`, source line 1775)
- `I.11` — Never transfer ownership by a raw pointer (`T*`) or reference (`T&`) (`ri-raw`, source line 1842)
- `I.12` — Declare a pointer that must not be null as `not_null` (`ri-nullptr`, source line 1903)
- `I.13` — Do not pass an array as a single pointer (`ri-array`, source line 1941)
- `I.22` — Avoid complex initialization of global objects (`ri-global-init`, source line 1997)
- `I.23` — Keep the number of function arguments low (`ri-nargs`, source line 2031)
- `I.24` — Avoid adjacent parameters that can be invoked by the same arguments in either order with different meaning (`ri-unrelated`, source line 2109)
- `I.25` — Prefer empty abstract classes as interfaces to class hierarchies (`ri-abstract`, source line 2163)
- `I.26` — If you want a cross-compiler ABI, use a C-style subset (`ri-abi`, source line 2202)
- `I.27` — For stable library ABI, consider the Pimpl idiom (`ri-pimpl`, source line 2220)
- `I.30` — Encapsulate rule violations (`ri-encapsulate`, source line 2268)
