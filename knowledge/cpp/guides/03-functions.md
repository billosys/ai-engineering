...
- `s-functions` — F: Functions (source lines 2331-4295)

## Rule Index

- `F.1` — "Package" meaningful operations as carefully named functions (`rf-package`, source line 2403)
- `F.2` — A function should perform a single logical operation (`rf-logical`, source line 2455)
- `F.3` — Keep functions short and simple (`rf-single`, source line 2515)
- `F.4` — If a function might have to be evaluated at compile time, declare it `constexpr` (`rf-constexpr`, source line 2599)
- `F.5` — If a function is very small and time-critical, declare it `inline` (`rf-inline`, source line 2653)
- `F.6` — If your function must not throw, declare it `noexcept` (`rf-noexcept`, source line 2687)
- `F.7` — For general use, take `T*` or `T&` arguments rather than smart pointers (`rf-smart`, source line 2753)
- `F.8` — Prefer pure functions (`rf-pure`, source line 2832)
- `F.9` — Unused parameters should be unnamed (`rf-unused`, source line 2847)
- `F.10` — If an operation can be reused, give it a name (`rf-name`, source line 2878)
- `F.11` — Use an unnamed lambda if you need a simple function object in one place only (`rf-lambda`, source line 2943)
- `F.15` — Prefer simple and conventional ways of passing information (`rf-conventional`, source line 2967)
- `F.16` — For "in" parameters, pass cheaply-copied types by value and others by reference to `const` (`rf-in`, source line 2993)
- `F.17` — For "in-out" parameters, pass by reference to non-`const` (`rf-inout`, source line 3051)
- `F.18` — For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter (`rf-consume`, source line 3098)
- `F.19` — For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter (`rf-forward`, source line 3139)
- `F.20` — For "out" output values, prefer return values to output parameters (`rf-out`, source line 3173)
- `F.21` — To return multiple "out" values, prefer returning a struct (`rf-out-multi`, source line 3238)
- `F.60` — Prefer `T*` over `T&` when "no argument" is a valid option (`rf-ptr-ref`, source line 3375)
- `F.22` — Use `T*` or `owner<T*>` to designate a single object (`rf-ptr`, source line 3408)
- `F.23` — Use a `not_null<T>` to indicate that "null" is not a valid value (`rf-nullptr`, source line 3464)
- `F.24` — Use a `span<T>` or a `span_p<T>` to designate a half-open sequence (`rf-range`, source line 3502)
- `F.25` — Use a `zstring` or a `not_null<zstring>` to designate a C-style string (`rf-zstring`, source line 3554)
- `F.26` — Use a `unique_ptr<T>` to transfer ownership where a pointer is needed (`rf-unique_ptr`, source line 3583)
- `F.27` — Use a `shared_ptr<T>` to share ownership (`rf-shared_ptr`, source line 3613)
- `F.42` — Return a `T*` to indicate a position (only) (`rf-return-ptr`, source line 3650)
- `F.43` — Never (directly or indirectly) return a pointer or a reference to a local object (`rf-dangle`, source line 3688)
- `F.44` — Return a `T&` when copy is undesirable and "returning no object" isn't needed (`rf-return-ref`, source line 3795)
- `F.45` — Don't return a `T&&` (`rf-return-ref-ref`, source line 3825)
- `F.46` — `int` is the return type for `main()` (`rf-main`, source line 3879)
- `F.47` — Return `T&` from assignment operators (`rf-assignment-op`, source line 3905)
- `F.48` — Don't `return std::move(local)` (`rf-return-move-local`, source line 3938)
- `F.49` — Don't return `const T` (`rf-return-const`, source line 3967)
- `F.50` — Use a lambda when a function won't do (to capture local variables, or to write a local function) (`rf-capture-vs-overload`, source line 3997)
- `F.51` — Where there is a choice, prefer default arguments over overloading (`rf-default-args`, source line 4032)
- `F.52` — Prefer capturing by reference in lambdas that will be used locally, including passed to algorithms (`rf-reference-capture`, source line 4067)
- `F.53` — Avoid capturing by reference in lambdas that will be used non-locally, including returned, stored on the heap, or passed to another thread (`rf-value-capture`, source line 4108)
- `F.54` — When writing a lambda that captures `this` or any class data member, don't use `[=]` default capture (`rf-this-capture`, source line 4143)
- `F.55` — Don't use `va_arg` arguments (`f-varargs`, source line 4183)
- `F.56` — Avoid unnecessary condition nesting (`f-nesting`, source line 4230)
