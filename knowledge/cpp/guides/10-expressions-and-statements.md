...
- `s-expr` — ES: Expressions and statements (source lines 10255-13846)
- `s-const` — Con: Constants and immutability (source lines 16929-17129)

## Rule Index

- `ES.1` — Prefer the standard library to other libraries and to "handcrafted code" (`res-lib`, source line 10342)
- `ES.2` — Prefer suitable abstractions to direct use of language features (`res-abstr`, source line 10373)
- `ES.3` — Don't repeat yourself, avoid redundant code (`res-dry`, source line 10410)
- `ES.5` — Keep scopes small (`res-scope`, source line 10452)
- `ES.6` — Declare names in for-statement initializers and conditions to limit scope (`res-cond`, source line 10512)
- `ES.7` — Keep common and local names short, and keep uncommon and non-local names longer (`res-name-length`, source line 10575)
- `ES.8` — Avoid similar-looking names (`res-name-similar`, source line 10645)
- `ES.9` — Avoid `ALL_CAPS` names (`res-not-caps`, source line 10672)
- `ES.10` — Declare one name (only) per declaration (`res-name-one`, source line 10703)
- `ES.11` — Use `auto` to avoid redundant repetition of type names (`res-auto`, source line 10761)
- `ES.12` — Do not reuse names in nested scopes (`res-reuse`, source line 10814)
- `ES.20` — Always initialize an object (`res-always`, source line 10907)
- `ES.21` — Don't introduce a variable (or constant) before you need to use it (`res-introduce`, source line 11095)
- `ES.22` — Don't declare a variable until you have a value to initialize it with (`res-init`, source line 11111)
- `ES.23` — Prefer the `{}`-initializer syntax (`res-list`, source line 11151)
- `ES.24` — Use a `unique_ptr<T>` to hold pointers (`res-unique`, source line 11253)
- `ES.25` — Declare an object `const` or `constexpr` unless you want to modify its value later on (`res-const`, source line 11283)
- `ES.26` — Don't use a variable for two unrelated purposes (`res-recycle`, source line 11304)
- `ES.27` — Use `std::array` or `stack_array` for arrays on the stack (`res-stack`, source line 11343)
- `ES.28` — Use lambdas for complex initialization, especially of `const` variables (`res-lambda-init`, source line 11387)
- `ES.30` — Don't use macros for program text manipulation (`res-macros`, source line 11417)
- `ES.31` — Don't use macros for constants or "functions" (`res-macros2`, source line 11484)
- `ES.32` — Use `ALL_CAPS` for all macro names (`res-all_caps`, source line 11508)
- `ES.33` — If you must use macros, give them unique names (`res-macros3`, source line 11524)
- `ES.40` — Avoid complicated expressions (`res-complicated`, source line 11629)
- `ES.41` — If in doubt about operator precedence, parenthesize (`res-parens`, source line 11698)
- `ES.42` — Keep use of pointers simple and straightforward (`res-ptr`, source line 11729)
- `ES.43` — Avoid expressions with undefined order of evaluation (`res-order`, source line 11928)
- `ES.44` — Don't depend on order of evaluation of function arguments (`res-order-fct`, source line 11952)
- `ES.45` — Avoid "magic constants"; use symbolic constants (`res-magic`, source line 11985)
- `ES.46` — Avoid lossy (narrowing, truncating) arithmetic conversions (`res-narrowing`, source line 12014)
- `ES.47` — Use `nullptr` rather than `0` or `NULL` (`res-nullptr`, source line 12066)
- `ES.48` — Avoid casts (`res-casts`, source line 12088)
- `ES.49` — If you must use a cast, use a named cast (`res-casts-named`, source line 12154)
- `ES.50` — Don't cast away `const` (`res-casts-const`, source line 12217)
- `ES.55` — Avoid the need for range checking (`res-range-checking`, source line 12396)
- `ES.56` — Write `std::move()` only when you need to explicitly move an object to another scope (`res-move`, source line 12413)
- `ES.60` — Avoid `new` and `delete` outside resource management functions (`res-new`, source line 12535)
- `ES.61` — Delete arrays using `delete[]` and non-arrays using `delete` (`res-del`, source line 12562)
- `ES.62` — Don't compare pointers into different arrays (`res-arr2`, source line 12586)
- `ES.63` — Don't slice (`res-slice`, source line 12610)
- `ES.64` — Use the `T{e}`notation for construction (`res-construct`, source line 12663)
- `ES.65` — Don't dereference an invalid pointer (`res-deref`, source line 12755)
- `ES.70` — Prefer a `switch`-statement to an `if`-statement when there is a choice (`res-switch-if`, source line 12904)
- `ES.71` — Prefer a range-`for`-statement to a `for`-statement when there is a choice (`res-for-range`, source line 12943)
- `ES.72` — Prefer a `for`-statement to a `while`-statement when there is an obvious loop variable (`res-for-while`, source line 12993)
- `ES.73` — Prefer a `while`-statement to a `for`-statement when there is no obvious loop variable (`res-while-for`, source line 13017)
- `ES.74` — Prefer to declare a loop variable in the initializer part of a `for`-statement (`res-for-init`, source line 13043)
- `ES.75` — Avoid `do`-statements (`res-do`, source line 13047)
- `ES.76` — Avoid `goto` (`res-goto`, source line 13070)
- `ES.77` — Minimize the use of `break` and `continue` in loops (`res-continue`, source line 13113)
- `ES.78` — Don't rely on implicit fallthrough in `switch` statements (`res-break`, source line 13193)
- `ES.79` — Use `default` to handle common cases (only) (`res-default`, source line 13258)
- `ES.84` — Don't try to declare a local variable with no name (`res-noname`, source line 13331)
- `ES.85` — Make empty statements visible (`res-empty`, source line 13358)
- `ES.86` — Avoid modifying loop control variables inside the body of raw for-loops (`res-loop-counter`, source line 13378)
- `ES.87` — Don't add redundant `==` or `!=` to conditions (`res-if`, source line 13409)
- `ES.100` — Don't mix signed and unsigned arithmetic (`res-mix`, source line 13494)
- `ES.101` — Use unsigned types for bit manipulation (`res-unsigned`, source line 13522)
- `ES.102` — Use signed types for arithmetic (`res-signed`, source line 13545)
- `ES.103` — Don't overflow (`res-overflow`, source line 13611)
- `ES.104` — Don't underflow (`res-underflow`, source line 13647)
- `ES.105` — Don't divide by integer zero (`res-zero`, source line 13670)
- `ES.106` — Don't try to avoid negative values by using `unsigned` (`res-nonnegative`, source line 13710)
- `ES.107` — Don't use `unsigned` for subscripts, prefer `gsl::index` (`res-subscripts`, source line 13775)
- `Con.1` — By default, make objects immutable (`rconst-immutable`, source line 16943)
- `Con.2` — By default, make member functions `const` (`rconst-fct`, source line 16979)
- `Con.3` — By default, pass pointers and references to `const`s (`rconst-ref`, source line 17059)
- `Con.4` — Use `const` to define objects with values that do not change after construction (`rconst-const`, source line 17085)
- `Con.5` — Use `constexpr` for values that can be computed at compile time (`rconst-constexpr`, source line 17110)
