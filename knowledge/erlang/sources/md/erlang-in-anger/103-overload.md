...
The best solution for this at the time of writing is to use [`lager`](https://github.com/basho/lager) as a substitute logging library.
...
One particularly bad example was in a pool manager for HTTP connections that I had written in a fork of the [`lhttpc`](https://github.com/ferd/lhttpc) library. It worked fine in most test cases we had, and we even had a connection timeout set to 10 milliseconds to be sure it never took too long[^3]. After a few weeks of perfect uptime, the HTTP client pool caused an outage when one of the remote servers went down.
...
There are also cases where the data is sent to you in a "fire and forget" manner — the entire system is part of an asynchronous pipeline — and it proves difficult to provide feedback to the end-user about why some requests were dropped or are missing. If you can reserve a special type of message that accumulates dropped responses and tells the user "`N` messages were dropped for reason `X`", that can, on its own, make the compromise far more acceptable to the user. This is the choice that was made with Heroku’s [logplex](https://devcenter.heroku.com/articles/logplex) log routing system, which can spit out [L10 errors](https://devcenter.heroku.com/articles/error-codes#l10-drain-buffer-overflow), alerting the user that a part of the system can’t deal with all the volume right now.
...
[^1]: Figuring out that a message queue is the problem is explained in Chapter Reading Crash Dumps, specifically in Section Full Mailboxes
...
[^9]: [Idempotence is Not a Medical Condition](http://queue.acm.org/detail.cfm?id=2187821), April 14, 2012
