...
[`../docs/PROJECT-MANAGEMENT.md`](../docs/PROJECT-MANAGEMENT.md): planning lays
...
(slice / arc / project). Restructured into a scale-free **invariant spine** (the
row format, the rules, the new `asserted < attested < reproduced < reconciled`
evidence-strength vocabulary, and the five-axes table of what adapts) followed
by three self-contained sections. Section A is the prior slice protocol,
reframed as the leaf scale and otherwise unchanged (CC/CDC protocols, the
five-iteration cap, the structural limitation, the per-slice template all
carry forward). Sections B and C are new: the arc and project tiers, with their
class-(a)/(b)/(c) rows, the *composition-must-be-reproduced* rule, the
doer/gatekeeper role shift, and *remediation-not-iteration* in place of the
fix-loop cap. Section C ships explicitly **provisional** — validated by analogy
and established practice, not yet by a closed project.
...
to the methodology (the level-1 ledger-bearing unit is **slice**, not
"milestone"; the ledger path is `arcNN-<slug>/sliceNN-<slug>/ledger.md`).
