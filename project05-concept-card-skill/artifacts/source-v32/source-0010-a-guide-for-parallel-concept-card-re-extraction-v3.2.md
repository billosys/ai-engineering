...
This guide documents how to re-extract concept cards from primary sources that already have an existing card inventory, upgrading them to the v3 template using an LLM with multiple agents working in parallel.
...
- Fixed the howto-guide reference in Prerequisites and in the Phase 2 agent prompt: the howto is `docs/dev/concept-cards/0009-howto-concept-card-extraction-with-llms-v3.2.md`, not `0004-concept-card-extraction-howto-v3.1.md` (which does not exist) or `crates/design/dev/skill/0004-...` (a legacy path that never matched this repo).
...
- **An LLM** installed and configured
...
- **Howto guide** available: reference `docs/dev/concept-cards/0009-howto-concept-card-extraction-with-llms-v3.2.md` for template and field definitions
...
- **Howto reference**: `docs/dev/concept-cards/0009-howto-concept-card-extraction-with-llms-v3.2.md`
...
- **v3.2**: Updated paths for the unified `ai-engineering` repo (`knowledge/<kb>/...`) and replaced `sources-<format>/` with `sources/<format>/` subdirectories; fixed the howto-guide reference to point at doc 0009 (not the nonexistent 0004); updated Phase 0 audit scripts to the new paths. No changes to the v3 frontmatter schema, body sections, or extraction machinery.
