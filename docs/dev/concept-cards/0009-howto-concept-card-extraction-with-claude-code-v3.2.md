# HOWTO: Concept Card Extraction with Claude Code (v3.2)
...
- Clarified that the markdown form at `knowledge/<kb>/sources/md/<source-slug>/` is the canonical extraction input, regardless of the original format (Markdown, PDF, or EPUB).
- Generalized the "Known Directory Issues" guidance into a slug-hygiene note, keeping the original music-theory example as an illustration.
- No changes to the v3 frontmatter schema, body sections, confidence scoring, typed relationships, or CQ framework — the template itself is untouched.

**v3.1 Changes (retained):**

- Re-extraction workflow for upgrading existing cards to the v3 template
- Merge-aware extraction steps (read old card, preserve unique content, overwrite)
- Known issues section (directory typos, LLM artifacts)
- Inherited v3 improvements: confidence scoring, typed relationships, CQ framework
...
- **source_slug**: The directory name for this source (the leaf under `sources/md/` and `concept-cards/`)
...
├── docs/
│   └── dev/
│       └── concept-cards/        # meta-level process docs (this file, etc.)
├── knowledge/
│   └── <kb>/
...
- The markdown form at `knowledge/<kb>/sources/md/<source-slug>/` is the canonical extraction input, regardless of the original format. PDF and EPUB sources are converted into Markdown chapter files (see docs 0005 and 0006) before extraction begins.
- Concept cards always write to `knowledge/<kb>/concept-cards/<source-slug>/`. The `<source-slug>` must match the one used under `sources/md/`.
- Extraction metadata (competency questions, extraction logs, CQ-to-chapter mappings) lives in `knowledge/<kb>/extraction-metadata/<source-slug>/`.
- Guides (prose style guides derived from extracted cards or standalone) live in `knowledge/<kb>/guides/`.

### Naming Conventions

| Element | Format | Example |
|---------|--------|---------|
| Knowledge-base slug | `<kb>` | `go`, `js`, `rust`, `design` |
| Source slug | `<source-slug>` | `21st-century-classroom`, `effective-go`, `uber` |
| Concept card file | `<concept-slug>.md` | `major-scale.md`, `error-wrapping.md` |
| Source directory (per format) | `sources/<format>/<source-slug>/` | `sources/md/uber/` |
| Output directory | `concept-cards/<source-slug>/` | `concept-cards/uber/` |
...
### Slug Hygiene & Known Issues

Before extracting, spot-check source and concept-card directory names for typos. A typo in `<source-slug>` causes silent breakage: cards end up in a directory that doesn't match `source_slug` in frontmatter, and cross-references fail.

**Illustrative example from past extractions:**

- `complete-muscician` -> `complete-musician` (missing `i`)
...
**General checks before starting extraction:**

- `<source-slug>` matches the expected title slug (no typos)
- `<source-slug>` is used consistently between `sources/md/<source-slug>/` and `concept-cards/<source-slug>/`
- Any `source_slug:` in existing frontmatter matches the actual leaf directory name
...
1. **The source material is primary** — always re-extract from the converted markdown in `knowledge/<kb>/sources/md/<source-slug>/`, not from the old card
...
1. **List existing cards** for this chapter's concepts in `knowledge/<kb>/concept-cards/<source-slug>/`
2. **Read the source chapter** in `knowledge/<kb>/sources/md/<source-slug>/`
...
5. **Before writing each card**, check if `knowledge/<kb>/concept-cards/<source-slug>/<slug>.md` already exists
...
- [ ] `source_slug` matches the directory name under both `sources/md/` and `concept-cards/`
...
### 11. Writing to the Wrong Knowledge Base

**Wrong**: Writing cards to `concept-cards/<source-slug>/` at the repo root (the old flat layout)
**Right**: Writing to `knowledge/<kb>/concept-cards/<source-slug>/` with `<kb>` chosen to match the domain
...
### Sources with No PDF (Pure Markdown or HTML-Origin)

Some sources were born in Markdown (blog posts, docs sites) or converted from HTML rather than PDF:

- Set `pdf_page: null`
- Cite by section heading or URL fragment in Source Reference
- Otherwise extraction is identical — the input is still `knowledge/<kb>/sources/md/<source-slug>/`
...
- [ ] Cards are under `knowledge/<kb>/concept-cards/<source-slug>/` (not legacy root paths)
...
- **v3.2**: Updated paths for the unified `ai-engineering` repo (`knowledge/<kb>/...`) and replaced `sources-<format>/` with `sources/<format>/` subdirectories; added guidance for non-PDF sources. No changes to the v3 frontmatter schema or body sections.
