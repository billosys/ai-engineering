...
| F-5 | Public vocabulary boundary remains preserved across guide wording and is recorded for Arc05 follow-up | `rg -n "public vocabulary boundary|Arc05|provisional|skill kind|atomic|composite|domain/tooling|framework/operational|method|protocol distribution|not finalized" artifacts/*.md` | serious | slice-plan | done | `artifacts/docs-content-boundary-evidence.md`; verifier command passes. | Vocabulary boundary evidence. |
| F-6 | Closing report walks all six rows, states source/planning status, and bubbles findings up to Arc04 | `test -f closing-report.md && rg -n "Rows: 6|Done: 6|source checkout|planning checkout|Bubble-Up to Arc04|Slice04|silent-drop|source commit" closing-report.md` | serious | slice-plan | done | `closing-report.md`; verifier command passes. | Slice close evidence. |

## Closure

Slice is verified-closed by CDC. The focused guide expansion, docs versus
knowledge boundary, README navigation preservation, source validation evidence,
and Arc05 vocabulary boundary were independently reproduced in
`cdc-verification.md`.
