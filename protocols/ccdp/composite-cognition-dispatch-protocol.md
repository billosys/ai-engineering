...
- `reason`: why the originating actor (Service or Dispatcher) is escalating (typed: PROVENANCE_BELOW_REQUIREMENT, CAPABILITY_EXCEEDED, DEADLINE_INSUFFICIENT, etc.)
- `achieved_grade`: the best Provenance Grade achieved or achievable, if any
...
- `escalation_origin`: whether the Escalation was generated by a Service (`"service"`) or by the Dispatcher (`"dispatcher"`)
...
- **Provenance:** If the Request specifies a `provenance_requirement` with a `min_policy_grade`, Services whose `typical_grade` meets or exceeds the grade threshold are preferred. When the requirement specifies `required_methods` or `required_evidence_types`, Services whose `provenance_capabilities` declare support for the requested methods and artifact types are preferred.
...
```json
{
  "audit_record": {
    ...
    "provenance_policy": {
      "selected_policy": "provenance_mismatch_policy",
...
| `provenance_policy.selected_policy` | — | — | S | — | — | — | — |
...
The `provenance_policy.*` and `escalation_routing.chain_source` fields are marked RECOMMENDED (S) because they apply only to Dispatcher-origin provenance Escalations, not to all ESCALATION records. However, Section 9.2 normatively requires these diagnostics for every Escalation generated by `provenance_mismatch_policy` or `provenance_unavailable_policy`: a conforming Dispatcher MUST record the selected policy, trigger, grade comparison, and — when applicable — missing methods, missing artifact types, and non-conformant Service identity. `escalation_routing.chain_source` indicates whether the escalation chain was sourced from the responding Service's capability record (`"responding_service"`) or routed directly to human review (`"human_review"`).
...
<a id="section-17-3"></a>
## NSA/CISA AI Deployment Security Baseline Mapping
